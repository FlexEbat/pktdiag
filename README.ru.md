...
- **Интерактивность TUI не проверена человеком.** `pktdiag tui`
  существует и собирается на GitHub Actions, как gopacket, Cobra и Viper
  до него. Ни эта песочница, ни headless-раннер CI не дают псевдотерминал
  для реального прогона интерактивного цикла, поэтому автоматически
  проверено только то, что `View()` рисует каждую вкладку без паники на
  реальных данных отчёта и что `Update()` правильно переключает вкладки
  (`internal/tui/tui_test.go`). Как это выглядит в настоящем терминале,
  человек ещё не смотрел.
