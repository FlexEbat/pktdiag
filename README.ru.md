...
- **Нет интерактивного мастера захвата.** Флаги и `pktdiag.yaml`
  заменяют пошаговые вопросы про интерфейс и фильтр из исходного дизайна.
- **Нет флага `--open`.** Исходный дизайн запускал Wireshark после
  захвата автоматически; эта версия пропускает это осознанно, она
  рассчитана на headless-использование.
- **Ограничение размера одного файла есть только для ring buffer.**
  `--ring-size` ограничивает каждый файл в кольце. Ограничить один файл
  без ротации не реализовано.
