...
`tcpdump`/`dumpcap`/`tshark`/`capinfos`/`zstd`/`wkhtmltopdf`, текущие
права (root или нет), свободное место на диске, доступные сетевые
интерфейсы. Возвращает ошибку, если чего-то обязательного (✘) не хватает;
отсутствие опциональных инструментов помечается как ⚠ и не блокирует
захват.
