...
| `tui` | Открывает интерактивный терминальный просмотр одного отчёта |
...
У pktdiag три внешние Go-зависимости: `github.com/google/gopacket` для
детекторов на уровне пакетов, которые display-filter не выразить,
`github.com/spf13/cobra` для маршрутизации команд и
`github.com/spf13/viper` для чтения `pktdiag.yaml`. Песочница, где
разрабатывался проект, блокирует `golang.org/x/*` на уровне сети,
поэтому сборки с этими зависимостями идут через GitHub Actions
(`.github/workflows/ci.yml`), у которого есть обычный доступ в
